...
describe('TurnTimer', () => {
  ...
  it('shows remaining seconds', () => {
    render(
      <TurnTimer turnTimeoutSecs={30} lastMoveAt={nowISO()} isOver={false} isSuspended={false} />,
    )
  ...
  it('does not go below 0', () => {
    render(
      <TurnTimer turnTimeoutSecs={2} lastMoveAt={nowISO()} isOver={false} isSuspended={false} />,
    )
    act(() => {
  ...
  it('hides when game is over', () => {
    render(
      <TurnTimer turnTimeoutSecs={30} lastMoveAt={nowISO()} isOver={true} isSuspended={false} />,
    )
  ...
  it('hides when timeout is 0', () => {
    render(
      <TurnTimer turnTimeoutSecs={0} lastMoveAt={nowISO()} isOver={false} isSuspended={false} />,
    )
    expect(screen.queryByTestId('turn-timer')).not.toBeInTheDocument()
  })
  ...
  it('freezes when suspended and does not tick', () => {
    render(
      <TurnTimer turnTimeoutSecs={30} lastMoveAt={nowISO()} isOver={false} isSuspended={true} />,
    )
  ...
  it('shows urgent style in last 10 seconds', () => {
    render(
      <TurnTimer turnTimeoutSecs={12} lastMoveAt={nowISO()} isOver={false} isSuspended={false} />,
    )
    act(() => {
      vi.advanceTimersByTime(3000)
    })
  ...
  it('does not show urgent style when paused even if time is low', () => {
    render(
      <TurnTimer turnTimeoutSecs={5} lastMoveAt={nowISO()} isOver={false} isSuspended={true} />,
    )
    const el = screen.getByTestId('turn-timer')
