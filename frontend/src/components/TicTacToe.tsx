...
export default function TicTacToeBoard({ state, currentPlayerId, localPlayerId, onMove, disabled }: Props) {
    ...
    <div className={styles.board}>
      {board.map((cell, i) => {
        ...
        return (
          ...
          >
            {cell !== '' && (
              <span className={`${styles.symbol} ${cell === 'X' ? styles.symbolX : styles.symbolO}`}>
                {cell}
              </span>
            )}
            {isEmpty && isMyTurn && (
              <span className={styles.hoverSymbol}>{localMark}</span>
            )}
          </button>
        )
      })}
