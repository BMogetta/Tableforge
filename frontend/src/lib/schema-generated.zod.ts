...
export const allowedEmailSchema = z.object({
  email: z.string(),
  role: z.enum(['player', 'manager', 'owner']),
...
export const banSchema = z.object({
  ...
  expires_at: z.string().datetime({ offset: true }).optional(),
  lifted_at: z.string().datetime({ offset: true }).optional(),
  lifted_by: z.string().optional(),
  created_at: z.string().datetime({ offset: true }),
})
...
export const botProfileSchema = z.object({
  ...
  determinizations: z.number().int(),
  exploration_c: z.number(),
...
export const dmConversationSchema = z.object({
  other_player_id: z.string(),
  other_username: z.string(),
  other_avatar_url: z.string().optional(),
...
export const friendshipViewSchema = z.object({
  friend_id: z.string(),
  friend_username: z.string(),
  friend_avatar_url: z.string().optional(),
  status: z.enum(['pending', 'accepted', 'blocked']),
  created_at: z.string().datetime({ offset: true }),
})
...
export const gameResultRecordSchema = z.object({
  id: z.string(),
  session_id: z.string(),
  game_id: z.string(),
  winner_id: z.string().optional(),
  is_draw: z.boolean(),
  ended_by: z.enum(['win', 'draw', 'forfeit', 'timeout', 'ready_timeout', 'suspended']),
...
export const gameSessionSchema = z.object({
  ...
  suspend_count: z.number().int(),
  suspended_at: z.string().datetime({ offset: true }).optional(),
  suspended_reason: z.string().optional(),
  ready_players: z.array(z.string()),
  turn_timeout_secs: z.number().int().optional(),
  last_move_at: z.string().datetime({ offset: true }),
...
export const leaderboardEntrySchema = z.object({
  ...
  username: z.string(),
  is_bot: z.boolean(),
  bot_profile: z.enum(['easy', 'medium', 'hard', 'aggressive']).optional(),
  display_rating: z.number(),
  games_played: z.number().int(),
})
export type LeaderboardEntry = z.infer<typeof leaderboardEntrySchema>

export const lobbySettingSchema = z.object({
  key: z.string(),
  label: z.string(),
  description: z.string().optional(),
  type: z.enum(['select', 'int']),
  default: z.string(),
...
export const matchHistoryEntrySchema = z.object({
  ...
  session_id: z.string(),
  game_id: z.string(),
  outcome: z.enum(['win', 'loss', 'draw', 'forfeit']),
  ended_by: z.enum(['win', 'draw', 'forfeit', 'timeout', 'ready_timeout', 'suspended']),
  duration_secs: z.number().int().optional(),
  created_at: z.string().datetime({ offset: true }),
  opponent_id: z.string().optional(),
  opponent_username: z.string().optional(),
  opponent_is_bot: z.boolean().optional(),
  opponent_bot_profile: z.enum(['easy', 'medium', 'hard', 'aggressive']).optional(),
})
...
export const notificationSchema = z.object({
  ...
  action_expires_at: z.string().datetime({ offset: true }).optional(),
  created_at: z.string().datetime({ offset: true }),
})
export type Notification = z.infer<typeof notificationSchema>

export const pauseVoteResultSchema = z.object({
  all_voted: z.boolean(),
  votes: z.number().int(),
...
export const playerMuteSchema = z.object({
  muter_id: z.string(),
  muted_id: z.string(),
  created_at: z.string().datetime({ offset: true }),
})
...
export const playerSettingMapSchema = z.object({
  theme: z.string().optional(),
  language: z.string().optional(),
  reduce_motion: z.boolean().optional(),
  font_size: z.string().optional(),
  notify_dm: z.boolean().optional(),
  ...
  volume_master: z.number().optional(),
  volume_sfx: z.number().optional(),
  volume_ui: z.number().optional(),
  volume_notifications: z.number().optional(),
  volume_music: z.number().optional(),
  show_move_hints: z.boolean().optional(),
  confirm_move: z.boolean().optional(),
  show_timer_warning: z.boolean().optional(),
...
export const rootaccessStateSchema = z
  .object({
    round: z.number().int().gte(1),
    phase: z.enum(['playing', 'debugger_pending', 'round_over', 'game_over']),
    players: z.array(z.string()),
    tokens: z.record(z.string(), z.number().int().gte(0)),
    eliminated: z.array(z.string()),
    protected: z.array(z.string()),
    backdoor_played_by: z.array(z.string()),
    discard_piles: z
      .record(z.string(), z.array(z.string()))
      .describe('Per-player discard history in order (oldest first).'),
    discard_order: z
      .array(z.object({ player_id: z.string(), card: z.string() }))
      .describe(
        'Global chronological discard order (oldest first). Populated by the backend in phase 2; absent means UI should fall back to per-player piles.',
      )
      .optional(),
    set_aside_visible: z.array(z.string()),
    ...
    round_winner_id: z
      .union([
        z
          .string()
          .describe(
            'Winner of the most recently completed round. Persists across round transitions so the client can render a round summary during the hand-off to the new round.',
          ),
        z
          .null()
          .describe(
      ...
      .optional(),
    last_round_backdoor_bonus_by: z
      .array(z.string())
      .describe(
        'Snapshot of backdoor_played_by at round end. Used by the round summary UI; separate from backdoor_played_by which resets each round.',
      )
      .optional(),
    game_winner_id: z.union([z.string(), z.null()]).optional(),
    debugger_choices: z.array(z.string()).optional(),
    private_reveals: z.record(z.string(), z.string()).optional(),
  })
  .describe(
    'Filtered view of Root Access engine state sent to clients. Hands for non-local players are server-filtered (length-only or empty).',
  )
export type RootaccessState = z.infer<typeof rootaccessStateSchema>
...
export const addAllowedEmailRequestSchema = z.object({
  email: z.string().min(1),
  role: z.enum(['player', 'manager', 'owner']).optional(),
})
export type AddAllowedEmailRequest = z.infer<typeof addAllowedEmailRequestSchema>

export const addBotRequestSchema = z.object({
  profile: z.enum(['easy', 'medium', 'hard', 'aggressive']).optional(),
})
...
export const adminBroadcastRequestSchema = z.object({
  message: z.string().min(1).max(500),
  type: z.enum(['info', 'warning']).optional(),
})
export type AdminBroadcastRequest = z.infer<typeof adminBroadcastRequestSchema>
...
export const createRoomRequestSchema = z.object({
  ...
  turn_timeout_secs: z.number().int().gte(5).optional(),
})
...
export const getPlayerRatingResponseSchema = z.object({
  player_id: z.string(),
  game_id: z.string(),
  display_rating: z.number(),
  games_played: z.number().int(),
  win_streak: z.number().int(),
  loss_streak: z.number().int(),
})
...
export const issueBanRequestSchema = z.object({
  reason: z.string().optional(),
...
export type ListAchievementDefinitionsResponse = z.infer<
  typeof listAchievementDefinitionsResponseSchema
>
...
export const voteRematchResponseSchema = z.object({
  votes: z.number().int(),
  total_players: z.number().int(),
})
