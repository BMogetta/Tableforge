...
export const playerSchema = z.object({ "id": z.string(), "username": z.string(), "role": z.enum(["player","manager","owner"]), "avatar_url": z.string().optional(), "is_bot": z.boolean(), "bot_profile": z.enum(["easy","medium","hard","aggressive"]).optional(), "created_at": z.string().datetime({ offset: true }), "deleted_at": z.string().datetime({ offset: true }).optional() })
export type Player = z.infer<typeof playerSchema>

export const playerAchievementSchema = z.object({ "id": z.string(), "player_id": z.string(), "achievement_key": z.string(), "tier": z.number().int(), "progress": z.number().int(), "unlocked_at": z.string().datetime({ offset: true }) })
...
export const roomPlayerSchema = z.object({ "id": z.string(), "username": z.string(), "role": z.enum(["player","manager","owner"]), "avatar_url": z.string().optional(), "is_bot": z.boolean(), "bot_profile": z.enum(["easy","medium","hard","aggressive"]).optional(), "created_at": z.string().datetime({ offset: true }), "deleted_at": z.string().datetime({ offset: true }).optional(), "seat": z.number().int(), "joined_at": z.string().datetime({ offset: true }) })
export type RoomPlayer = z.infer<typeof roomPlayerSchema>

export const rootaccessStateSchema = z.object({ "round": z.number().int().gte(1), "phase": z.enum(["playing","debugger_pending","round_over","game_over"]), "players": z.array(z.string()), "tokens": z.record(z.string(), z.number().int().gte(0)), "eliminated": z.array(z.string()), "protected": z.array(z.string()), "backdoor_played_by": z.array(z.string()), "discard_piles": z.record(z.string(), z.array(z.string())).describe("Per-player discard history in order (oldest first)."), "discard_order": z.array(z.object({ "player_id": z.string(), "card": z.string() })).describe("Global chronological discard order (oldest first). Populated by the backend in phase 2; absent means UI should fall back to per-player piles.").optional(), "set_aside_visible": z.array(z.string()), "deck": z.array(z.string()), "hands": z.record(z.string(), z.array(z.string())), "round_winner_id": z.union([z.string().describe("Winner of the most recently completed round. Persists across round transitions so the client can render a round summary during the hand-off to the new round."), z.null().describe("Winner of the most recently completed round. Persists across round transitions so the client can render a round summary during the hand-off to the new round.")]).describe("Winner of the most recently completed round. Persists across round transitions so the client can render a round summary during the hand-off to the new round.").optional(), "last_round_backdoor_bonus_by": z.array(z.string()).describe("Snapshot of backdoor_played_by at round end. Used by the round summary UI; separate from backdoor_played_by which resets each round.").optional(), "game_winner_id": z.union([z.string(), z.null()]).optional(), "debugger_choices": z.array(z.string()).optional(), "private_reveals": z.record(z.string(), z.string()).optional() }).describe("Filtered view of Root Access engine state sent to clients. Hands for non-local players are server-filtered (length-only or empty).")
...
export const adminBroadcastRequestSchema = z.object({ "message": z.string().min(1).max(500), "type": z.enum(["info","warning"]).optional() })
