...
func (svc *Service) MaybeFireBot(ctx context.Context, hub *ws.Hub, sessionID uuid.UUID, currentPlayerID uuid.UUID) {
	...
	go func() {
		...
		if err := json.Unmarshal(session.State, &state); err != nil {
			log.Printf("bot: MaybeFireBot: decode state for session %s: %v", sessionID, err)
			return
		}
		...
		if err != nil {
			...
			log.Printf("bot: MaybeFireBot: DecideMove session %s player %s: %v",
				sessionID, bp.ID, err)
			return
		}
		...
		if err != nil {
			...
			log.Printf("bot: MaybeFireBot: ApplyMove session %s player %s: %v",
				sessionID, bp.ID, err)
			return
		}
