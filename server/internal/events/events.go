...
func (s *Store) Append(ctx context.Context, sessionID uuid.UUID, eventType Type, playerID *uuid.UUID, payload any) {
	...
	if err != nil {
		log.Printf("events.Append: marshal payload for %s/%s: %v", sessionID, eventType, err)
		return
	}
	...
	if err := s.rdb.XAdd(ctx, &redis.XAddArgs{
		...
		log.Printf("events.Append: XAdd %s/%s: %v", sessionID, eventType, err)
	}
}
...
func (s *Store) Persist(ctx context.Context, sessionID uuid.UUID) {
	...
	if err != nil {
		log.Printf("events.Persist: XRange %s: %v", sessionID, err)
		return
	}
	if len(entries) == 0 {
		return
	...
	for _, entry := range entries {
		...
		if err != nil {
			log.Printf("events.Persist: parse entry %s: %v", entry.ID, err)
			continue
		}
	...
	if err := s.st.BulkCreateSessionEvents(ctx, params); err != nil {
		log.Printf("events.Persist: bulk insert for session %s: %v", sessionID, err)
		return
	}

	// Stream is no longer needed after persistence.
	if err := s.rdb.Del(ctx, key).Err(); err != nil {
		log.Printf("events.Persist: DEL stream %s: %v", sessionID, err)
	}
}
...
func (s *Store) Read(ctx context.Context, sessionID uuid.UUID) ([]Event, error) {
	...
	if err != nil {
		log.Printf("events.Read: EXISTS %s: %v", sessionID, err)
...
func (s *Store) readFromStream(ctx context.Context, sessionID uuid.UUID) ([]Event, error) {
	...
	for _, entry := range entries {
		p, err := parseEntry(sessionID, entry)
		if err != nil {
			log.Printf("events.Read: parse entry %s: %v", entry.ID, err)
			continue
		}
