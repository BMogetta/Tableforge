...
- [~] **Phase 4** — `shared/contracts.md` updated ✓, memory `project_redis_pubsub_scaling.md` superseded ✓, replicaCount bumps landed in `recess-deploy` ✓. Deferred: Asynqmon UI deployment (upstream `hibiken/asynqmon` is amd64-only and the multi-arch mirror build kept hitting stack-specific snags; not worth more cycles on a nice-to-have ops UI when the 3 Asynq queues are inspectable via `kubectl` + `redis-cli`. Mirror workflow + custom Dockerfile remain in the repo dormant — see `.github/workflows/asynqmon-mirror.yml` and `.github/docker/asynqmon.Dockerfile`). Optional follow-up: consumer-lag + PEL-size Prometheus metrics in `shared/streams/`.
