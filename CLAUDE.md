...
- **Orchestrator (you):** stays on `main`. Plans, writes briefs, reviews diffs,
  merges, runs post-merge verification. Never writes application code directly.
- **Worktree agents:** each receives a self-contained brief, works in an
  isolated git worktree on a dedicated branch, commits, runs tests, and reports.

#### Branch naming

Format: `<type>/<scope>-<short-description>` — max 40 characters.

| Segment | Rules |
|---------|-------|
| `type` | `feat`, `fix`, `refactor`, `test`, `chore` |
| `scope` | Shortest unambiguous name for the service/directory. Examples: `auth`, `game`, `sfx`, `db` |
| `short-description` | 2–4 words, kebab-case |

`isolation: "worktree"` creates branches as `worktree-agent-{hash}`. The agent
must rename its branch as the **first step** before writing any code:
```bash
git branch -m <new-branch-name>
```
Include the target branch name in the brief so the agent knows what to rename to.

If two agents touch the same scope in a batch, add a noun suffix to
differentiate: `feat/auth-oauth-google` vs `feat/auth-session-expiry`.

#### Lifecycle

1. **Plan** — define tasks, file scope, acceptance criteria. Build the
   ownership matrix before dispatching any agent.
2. **Launch** — dispatch agent with `isolation: "worktree"`.
3. **Agent works** — renames branch, implements, commits (conventional commit),
   runs tests, reports results.
4. **Scope check** — orchestrator verifies the diff only touches allowed files.
5. **Merge** — `git merge --no-ff` to main (atomic, revertible merge commit).
6. **Post-merge verify** — run tests on main.
7. **Clean up** — remove worktree and delete branch.

#### Brief format

Every agent brief must include these sections:

```
## Branch name
The target branch name (agent renames as first step).

## Task
What to implement and why.

## Context
Files to read for reference (do not modify). Example:
  - services/auth/internal/middleware.go (understand auth pattern)
  - shared/events/events.go (understand event struct conventions)

## Allowed files
Files and directories the agent may create or modify.
Use glob patterns. Example:
  - services/user-service/internal/achievements/**
  - shared/db/migrations/004_admin.sql
  - shared/events/events.go (append only — add new event struct)

## Do NOT touch
Files explicitly off-limits (to prevent conflicts with other agents).

## Acceptance criteria
What "done" looks like — expected behavior, test coverage.

## Testing
- Unit tests: required. Agent must write and run them before finishing.
- Integration / e2e tests: if not feasible in the worktree, leave a
  TODO comment in the test file describing what to test and how.
- Run the applicable test suites before committing:
  Go changes: go test ./...
  Frontend changes: cd frontend && npm test && npx tsc --noEmit
  Both: all three commands.

## Commit
Conventional commit message to use.
```

#### Ownership matrix

Define this **before** launching any agents in a batch. Format:

| Agent | Branch | Allowed paths | Shared files (append-only) |
|-------|--------|---------------|----------------------------|
| A | `feat/auth-oauth` | `services/auth/**` | `shared/events/events.go` |
| B | `fix/orders-dup` | `services/orders/**` | — |

Rules:
- **No two concurrent agents share writable paths.** If overlap exists,
  serialize those agents instead of running them in parallel.
- **Shared files are append-only.** The agent may only add new
  structs/types/constants — never modify existing ones.
- `shared/` files (events, proto, schemas, migrations) are high-conflict zones.
  If multiple agents need them, either serialize or give each one a specific
...
```
If the diff contains files outside the allowed list:
1. Do **not** merge.
2. Preferred: ask the agent to revert out-of-scope changes and re-commit.
3. Last resort: cherry-pick only valid commits onto a clean branch. This is a
   process failure signal — tighten the brief before the next batch.

#### Merge protocol

1. **Never merge stale branches.** If main has advanced since the agent
   branched, rebase first:
   ```bash
   git rebase main <branch>
   ```
   - Conflicts in files **outside** agent's allowed paths → discard branch,
     re-launch agent from fresh HEAD.
   - Conflicts **inside** allowed paths → orchestrator resolves in worktree,
     re-runs tests before merging.
2. **Always use `--no-ff`:**
   ```bash
   git merge --no-ff <branch>
   ```
3. **Never cherry-pick as default strategy.** Only as last resort for branches
   with out-of-scope changes (process failure).
4. **Stash or commit local changes** before any merge/rebase operation.

#### Sequencing

- **Never launch new agents while unmerged branches exist.** Merge all
  completed branches to main first so new agents branch from the latest HEAD.
- For a batch of N independent tasks: launch all N in parallel, then merge
  them one by one (merge → verify → clean up → next merge).
- If a batch has dependencies (task B needs task A's output), launch A first,
  merge it, then launch B.
...
|--------------|----------|
| Go only | `go test ./...` |
| Frontend only | `cd frontend && npm test && npx tsc --noEmit` |
| Both / unsure | All three — default to this when in doubt |

1. **Agent tests in worktree** — runs all applicable commands before
   committing. Reports: commands run, pass/fail counts, error output.
2. **Orchestrator verifies after merge** — always re-run tests on main. Never
   trust the agent report as a substitute.
...
| Situation | Action |
|-----------|--------|
| Fix is small (< 10 lines) in agent's files | Fix directly on main after revert |
| Fix requires meaningful changes | Re-launch agent with updated brief describing the failure. New branch with `-v2` suffix. |
| Failure in files agent shouldn't have touched | Scope check missed it — tighten ownership matrix |

**Step 3 — Clean up** the failed branch, then re-run lifecycle from step 2.

#### When it is OK to work directly on `main`

All three conditions must be met:
1. The change touches no business logic (no functions, classes, or conditionals).
2. No new tests are required.
3. The change is documentation (`.md`), static config (`.yaml`, `.toml`,
   `.env.example`), or dependency manifests (`go.mod` tidy, `package.json`
   version bumps).
